...
static void draw_building_top(int grid_offset, building *b, int x, int y)
{
    ...
    if (b->type == BUILDING_GRANARY) {
        ...
        if (b->data.granary.resource_stored[RESOURCE_NONE] < 3200) {
            ...
            if (b->data.granary.resource_stored[RESOURCE_NONE] < 1600) {
                image_draw(image_group(GROUP_BUILDING_GRANARY) + 4, x + 91, y - 50);
            }
            if (b->data.granary.resource_stored[RESOURCE_NONE] < 800) {
